...
P\home\Главная страница\P16\Хинт страницы (показывается без линии отводки)
  F\100%x100%\OUT:LT\P12\Хинт контейнера
    C\Заголовок\x40\OUT:LT:M4\P6\Хинт надписи
    B\Перейти на users\ICON:chevron-right\OUT:RB:M6\140x32\P8\GOTO:users\Хинт кнопки (переход)
    T\COLS:20R,50L,30\100%x60%\OUT:B:M10\P6\Хинт таблицы
      TH\ID\ФИО\Роль
      TD\1\Алексей\Админ
P\users\Пользователи\Хинт второй страницы
  C\Вы на странице users\x20\OUT:T:M4\P6\Хинт надписи на второй странице
  B\Назад на home\ICON:back\OUT:L:M6\120x32\P8\GOTO:home\Хинт кнопки назад
```
````
...
Допустимые комбинации: `OUT:LT`, `OUT:LB`, `OUT:RT`, `OUT:RB`, `OUT:L`, `OUT:R`, `OUT:T`, `OUT:B` (и любая из них может иметь суффикс `:M<number>`). Пусто — центр.
...
## M / P — отступы (margin / padding)

Дополнительные поля (целые числа в пикселях, **одно значение на все стороны**):

- `M<number>` — **margin** (внешний отступ, влияет на layout)
- `P<number>` — **padding** (внутренний отступ, влияет на layout)
...
- По умолчанию:
  - `C`: `M3`, `P5`
  - `B`: `M3`, `P5`
  - `T`: `P5` (padding ячеек `TH/TD`)
  - остальные случаи: `M0`, `P0`
- Размеры `SIZE` считаются как **border-box**: padding “внутри” заданного размера.

Где используются:

- `P` — поддерживает только `P<number>`
- `F` — поддерживает только `P<number>`
- `C`, `B` — поддерживают `M<number>` и `P<number>`
- `T` — поддерживает `M<number>` (внешний отступ таблицы) и `P<number>` (**padding ячеек `TH/TD`**)
...
- `P\users\Главная страница\P16\Подсказка страницы`
...
Поддерживаются: `SIZE`, `ALIGN`, `P<number>`, `HINT`.
...
- `F\100%x80%\T\P12\BG:src/yarmap.PNG\Подсказка контейнера`
...
- `ICON:NAME`: встроенная пиктограмма слева от текста (без внешних файлов)
  - доступные `NAME`: `ai`, `arrow-left`, `arrow-right`, `back`, `box`, `chevron-left`, `chevron-right`, `close`, `codepen`, `cpu`, `edit`, `grid`, `home`, `loader`, `microphone`, `plus`, `search`, `sun`, `settings`, `trash`, `volume-2`
...
- `B\Перейти\ICON:search\OUT:RB:M6\140x32\P8\R8\GOTO:users\Открыть страницу users`
- `B\Перейти\ICON:search:18\OUT:RB:M6\140x32\P8\R8\GOTO:users\Открыть страницу users`
...
- `C\Текст\x20\OUT:LT:M4\P6\GOTO:users\Подсказка`
...
- `T\COLS:20R,50L,30\100%x60%\OUT:T:M10\P6\Подсказка таблицы`
