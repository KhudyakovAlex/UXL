...
Поддерживаются: `SIZE`, `ALIGN`, `IN:M<number>`, `IN:<ALIGN>`, `IN:H|IN:V`, `IN:L|IN:C|IN:R`, `HINT`.
...
- `ICON:NAME`: встроенная пиктограмма слева от текста (без внешних файлов)
  - доступные `NAME`: `ai`, `arrow-left`, `arrow-right`, `back`, `box`, `chevron-left`, `chevron-right`, `close`, `codepen`, `cpu`, `edit`, `grid`, `grid1x1`, `grid3x3`, `grid4x4`, `home`, `loader`, `microphone`, `plus`, `search`, `sliders`, `sun`, `settings`, `trash`, `volume-2`, `volume-x`
...
- В демо‑рендере клик по `S` просто **переключает активный вариант по кругу**.
- Никаких действий (`GOTO`) и сохранения состояния пока нет.
