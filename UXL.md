...
P\home\Главная страница\P16\Хинт страницы (показывается без линии отводки)
  F\100%x100%\OUT:LT\P12\Хинт контейнера
    C\Заголовок\x40\OUT:LT\P6\M4\Хинт надписи
    B\Перейти на users\ICON:chevron-right\OUT:RB\140x32\P8\M6\GOTO:users\Хинт кнопки (переход)
    T\COLS:20R,50L,30\100%x60%\OUT:B\M10\P6\Хинт таблицы
      TH\ID\ФИО\Роль
      TD\1\Алексей\Админ
P\users\Пользователи\Хинт второй страницы
  C\Вы на странице users\x20\OUT:T\P6\M4\Хинт надписи на второй странице
  B\Назад на home\ICON:back\OUT:L\120x32\P8\M6\GOTO:home\Хинт кнопки назад
```
````
...
Выравнивание задаётся токеном вида `OUT:<ALIGN>` (например `OUT:LT`).
...
Допустимые комбинации: `OUT:LT`, `OUT:LB`, `OUT:RT`, `OUT:RB`, `OUT:L`, `OUT:R`, `OUT:T`, `OUT:B`. Пусто — центр.
...
- `I\SRC:src/yarmap.PNG\200x120\LT\FIT\M6\Подсказка картинки`
- `I\SRC:src/yarmap.PNG\200x120\LT\FIT\M6\R12\Подсказка картинки`
...
- `C\Текст\LT\x20\P6\M4\GOTO:users\Подсказка`
...
- `T\COLS:20R,50L,30\100%x60%\T\M10\P6\Подсказка таблицы`
