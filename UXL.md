...
- `TYPE:G:NEW` — метки типа (можно указывать несколько через `:`):
  - `G` — "глобальная" страница для карты интерфейса: в **карте интерфейса** ставится отдельным списком **внизу** и считается **без связей** (стрелки к/от неё не рисуются); на рендер/прототип (GOTO/GOTOBACK) это поле **не влияет**
  - `NEW` — в правом нижнем углу элемента отрисовывается бейдж "новинка" (иконка `new` на красном фоне `#c90000`)
  - Пример: `TYPE:G:NEW` (обе метки), `TYPE:G` (только глобальная), `TYPE:NEW` (только бейдж)
  - **Поддерживается у всех тегов** (`P`, `F`, `B`, `C`, `S`, `I`, `T`)
