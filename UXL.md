...
Поля после `SRC:` могут идти **в любом порядке** и распознаются по виду:
...
- `FIT:contain`: вписать картинку в заданный `SIZE` (сохраняя пропорции)
- `FIT:cover`: заполнить `SIZE` (сохраняя пропорции, обрезая лишнее)
- если `FIT` **не указан** — **без масштабирования** (картинка центрируется и обрезается по границам элемента)
- `M<number>`: margin картинки
- всё остальное — `HINT`

Пример (все поля):

- `I\SRC:src/yarmap.PNG\200x120\LT\FIT:contain\M6\Подсказка картинки`
